import logging
from telegram import Update, ReplyKeyboardMarkup
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes

# === Логирование ===
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO
)
logger = logging.getLogger(__name__)

# === Ссылки ===
PROGRAM_LINK = "https://disk.yandex.ru/i/cbNiheeSFy6NvQ"
PROTOCOLS_LINK = "https://disk.yandex.ru/d/jthWuv1YfJatPw"
CHART_IMAGE_URL = "https://disk.yandex.ru/i/WKywmfTxGmfZFg"

# === Меню ===
MAIN_MENU = [
    "ИСМП",
    "Санэпидрежим в подразделениях",
    "Дезинфекционный режим в подразделениях",
    "Микробиологический мониторинг",
    "Мероприятия при выявлении инфекционного больного",
    "Аварийные ситуации",
    "Гигиена рук",
    "Стратифицированные показатели",
    "Внутренний контроль",
    "Особо опасные инфекции",
]

ISM_MENU = [
    "Комиссия по профилактике ИСМП",
    "Критерии ИСМП",
    "Профилактика ИСМП у пациентов и персонала",
    "Аббревиатуры",
    "Стандартные определения случаев ИСМП",
    "Программа профилактики ИСМП",
    "Выявленные ИСМП"
]

CRITERIA_MENU = ["У сотрудников", "У пациентов"]
SOS_MENU = [
    "ИАИ - имплантат-ассоциированная инфекция",
    "ИДП – инфекции дыхательных путей",
    "ИМП - инфекция мочевыводящих путей",
    "ИОХВ - инфекция области хирургического вмешательства",
    "ИК - инфекции кровотока"
]

IAI_MENU = [
    "Определение ИАИ",
    "Признаки ИАИ",
    "Факторы риска развития ППИ",
    "Типы ППИ",
    "Вероятные пути инфицирования ППИ",
    "Действия в случае выявления ИАИ"
]

IDD_MENU = ["Определение ИОД", "Критерии ИОД", "Действия персонала при подозрении на ИОД"]
IDD_DEF_MENU = [
    "Пневмония, связанная с оказанием медицинской помощи",
    "Трахеобронхиальные инфекции (бронхит, трахеобронхит, бронхиолит, трахеит без признаков пневмонии)",
    "Прочие инфекции нижних дыхательных путей"
]
IDD_CRIT_MENU = [
    "пневмонии без микробиологического подтверждения",
    "пневмонии микробиологически подтвержденные",
    "ИВЛ-ассоциированные пневмонии"
]

IMP_MENU = ["Определение ИМП", "Виды и критерии ИМП", "Действия персонала при выявлении ИМП"]
IMP_TYPES_MENU = [
    "микробиологически подтвержденная симптоматическая ИМП",
    "не подтвержденная микробиологически симптоматическая ИМП",
    "катетер-ассоциированная ИМП"
]

IOHV_MENU = ["Определение ИОХВ", "Виды и критерии ИОХВ", "Действия персонала при выявлении ИОХВ"]
IOHV_TYPES_MENU = [
    "Поверхностная инфекция области хирургического вмешательства",
    "Глубокая инфекция области хирургического вмешательства",
    "Инфекция области хирургического вмешательства органа или полости"
]

IK_MENU = ["Определения ИК", "Виды и критерии ИК", "Действия персонала при выявлении ИК"]
IK_TYPES_MENU = [
    "Лабораторно подтвержденная инфекция кровотока",
    "Катетер-ассоциированная инфекция кровотока (КАИК)",
    "ИК связанная с ЦВК (без положительного высева из крови)",
    "ИК ПВК (без положительного высева из крови)",
    "Генерализованная ИК, связанная с ЦВК (без положительного высева из крови)",
    "Генерализованная ИК, связанная с ПВК (без положительного высева из крови)",
    "Микробиологически подтвержденная инфекция кровотока, связанная с ЦВК",
    "Микробиологически подтвержденная инфекция кровотока, связанная с ПВК"

]

MM_MENU = [
    "Группы микроорганизмов (пациенты)",
    "Вид микроорганизмов (пациенты)",
    "Профили резистентности пациентов",
    "Нормативные документы",
    "Производственный контроль",
    "Перечень биоматериала для планового микробиологического исследования",
    "Годовой отчет ММ",
    "Оценка эффективности дезинфицирующих средств",
    "Частота ESCAPE-патогенов",
    "Коэффициент микробного разнообразия",
    "Пациенты с полирезистентными штаммами"
]

MM_GROUP_MO_MENU = ["2024 год группы МО", "2025 год группы МО"]
MM_SPECIES_MO_MENU = ["2024 год вид МО", "2025 год вид МО"]
MM_RESIST_MENU = ["2024 пр резист", "2025 пр резист"]
MM_ESCAPE_MENU = ["2024 Escape", "2025 Escape"]

# === Аварийные ситуации ===
AS_MENU = [
    "Алгоритм мер личной профилактики",
    "Алгоритм учета и регистрации аварийных ситуаций",
    "Состав укладки экстренной профилактики парентеральных инфекций",
    "Дотестовое и послетестовое консультирование при экспресс тестировании на ВИЧ-инфекцию",
    "Информирование о проведении тестирования на ВИЧ-инфекцию",
    "Журнал учета аварийных ситуаций",
    "Мероприятия по снижению числа АС"
]

AS_CONSULT_MENU = [
    "Дотестовое консультирование",
    "Послетестовое консультирование",
    "Схема послетестового консультирования при отрицательном результате",
    "Схема послетестового консультирования при положительном результате"
]

DETECTED_MENU = ["Стационар + Хирургическое отделение", "ОАРИТ", "Обобщенные данные"]

# === Меню: Особо опасные инфекции ===
OOI_MENU = [
    "Противоэпидемические мероприятия",
    "СИЗ для ООИ",
    "Лечение и экстренная профилактика при ООИ",
    "Меры личной профилактики медицинских работников",
    "Забор биоматериала при подозрении на ООИ",
    "Режимы обеззараживания поверхностей при ООИ"
]

OOI_MEASURES_MENU = [
    "Общий план мероприятий",
    "Первичные мероприятия"
]

OOI_PPE_MENU = [
    "Виды костюмов",
    "Случаи применения костюмов",
    "Порядок надевания и снятия костюма «1 типа»",
    "Порядок надевания и снятия «Кварц-М1»"
]

OOI_PPE_TYPES_MENU = [
    "Костюм 1 типа",
    "Костюм 2 типа",
    "Костюм 3 типа",
    "Костюм 4 типа",
    "Костюм Кварц-М1"
]

OOI_TREATMENT_MENU = [
    "Схема терапии больных в критическом состоянии",
    "Схема профилактики при неизвестном возбудителе",
    "Чума",
    "Холера",
    "Патогенный грипп",
    "Тропическая малярия",
    "Лихорадка Рифт-Валли",
    "Менингококковая болезнь"
]

OOI_PERSONAL_MENU = [
    "Меры",
    "Укладка экстренной профилактики ООИ"
]

OOI_DECONT_MENU = [
    "Холера",
    "Чума",
    "Сибирская язва",
    "Фекалии, мокрота, сыворотка крови",
    "Вирусные инфекции",
    "Приготовление раствора «Хлорапин»"
]
# === Мероприятия при выявлении инфекционного больного ===
INFECT_MENU = [
    "Первичные противоэпидемические мероприятия",
    "Действия в случае выявления ВИЧ-инфекции у пациента",
    "Действия при выявлении гепатита В и С у пациента",
    "Действия в случае выявления туберкулеза у пациента",
    "Действия в случае выявления сальмонеллеза у пациента",
    "Действия в случае выявления Гепатита А и Е",
    "Противоэпидемические мероприятия против Грипп, ОРВИ, Covid-19",
    "Действия при выявлении кори, краснухи, эпидпаротита у пациента",
    "Действия при выявлении коклюша у пациента",
    "Действия при выявлении дифтерии у пациента",
    "Действия в случае выявления менингококковой инфекции",
    "Действия в случае выявления пневмонии у пациента",
    "Действия при выявлении стрептококковой инфекции группы А у пациента",
    "Действия в случае выявления легионеллеза",
    "Действия при выявлении гнойно-воспалительных заболеваний",
]

# === Клавиатура: по одной кнопке на строку ===
def build_menu(buttons, back=True, home=True):
    menu = [[btn] for btn in buttons]
    bottom = []
    if back:
        bottom.append("Назад")
    if home:
        bottom.append("В начало")
    if bottom:
        menu.append(bottom)
    return menu

# === Отправка длинных сообщений ===
async def reply_safe(update: Update, text: str, reply_markup=None):
    max_len = 4000
    if len(text) <= max_len:
        await update.message.reply_text(text, reply_markup=reply_markup, parse_mode="HTML")
    else:
        parts = [text[i:i + max_len] for i in range(0, len(text), max_len)]
        for part in parts[:-1]:
            await update.message.reply_text(part, parse_mode="HTML")
        await update.message.reply_text(parts[-1], reply_markup=reply_markup, parse_mode="HTML")

# === /start ===
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    mk = ReplyKeyboardMarkup(build_menu(MAIN_MENU), resize_keyboard=True)
    await update.message.reply_text("Выберите раздел:", reply_markup=mk)
    context.user_data["menu_stack"] = ["main"]

# === Обработка сообщений ===
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    stack = context.user_data.setdefault("menu_stack", ["main"])

    if text == "В начало":
        stack.clear()
        stack.append("main")
        mk = ReplyKeyboardMarkup(build_menu(MAIN_MENU), resize_keyboard=True)
        await update.message.reply_text("Вы в главном меню.", reply_markup=mk)
        return

    if text == "Назад":
        if len(stack) > 1:
            stack.pop()
        current = stack[-1]
        nav = {
            "main": (MAIN_MENU, "Вы в главном меню."),
            "ism_menu": (MAIN_MENU, "Раздел ИСМП"),
            "criteria": (ISM_MENU, "Выберите пункт в ИСМП:"),
            "sos": (ISM_MENU, "Выберите пункт в ИСМП:"),
            "iai": (SOS_MENU, "Выберите тип инфекции:"),
            "idd": (SOS_MENU, "Выберите тип инфекции:"),
            "idd_def": (IDD_MENU, "Определение ИОД:"),
            "idd_crit": (IDD_MENU, "Критерии ИОД:"),
            "imp": (SOS_MENU, "Выберите тип инфекции:"),
            "imp_types": (IMP_MENU, "ИМП:"),
            "iohv": (SOS_MENU, "Выберите тип инфекции:"),
            "iohv_types": (IOHV_MENU, "ИОХВ:"),
            "ik": (SOS_MENU, "Выберите тип инфекции:"),
            "ik_types": (IK_MENU, "Инфекции кровотока:"),
            "detected": (ISM_MENU, "Выберите пункт в ИСМП:"),
            "oOI_menu": (OOI_MENU, "Особо опасные инфекции:"),
            "oOI_measures": (OOI_MEASURES_MENU, "Противоэпидемические мероприятия:"),
            "oOI_ppe": (OOI_PPE_MENU, "СИЗ для ООИ:"),
            "oOI_ppe_types": (OOI_PPE_TYPES_MENU, "Виды костюмов:"),
            "oOI_treatment": (OOI_TREATMENT_MENU, "Лечение и профилактика:"),
            "oOI_personal": (OOI_PERSONAL_MENU, "Меры личной профилактики:"),
            "oOI_decont": (OOI_DECONT_MENU, "Режимы обеззараживания:"),
        }
        if current in nav:
            btns, msg = nav[current]
            mk = ReplyKeyboardMarkup(build_menu(btns), resize_keyboard=True)
            await update.message.reply_text(msg, reply_markup=mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(MAIN_MENU), resize_keyboard=True)
            await update.message.reply_text("Возврат в главное меню.", reply_markup=mk)
            stack.clear()
            stack.append("main")
        return

    # === Главное меню ===
    if stack[-1] == "main":
        if text == "ИСМП":
            stack.append("ism_menu")
            mk = ReplyKeyboardMarkup(build_menu(ISM_MENU), resize_keyboard=True)
            await update.message.reply_text("Раздел ИСМП:", reply_markup=mk)
        elif text == "Санэпидрежим в подразделениях":
            link = "https://disk.yandex.ru/d/_tb-gEeevGXGZQ"
            await update.message.reply_text(f"📄 Санэпидрежим в подразделениях:\n{link}")
            mk = ReplyKeyboardMarkup(build_menu(MAIN_MENU), resize_keyboard=True)
            await update.message.reply_text("Вы в главном меню.", reply_markup=mk)
        elif text == "Дезинфекционный режим в подразделениях":
            link = "https://disk.yandex.ru/d/SyYix-yb09jfYQ"
            await update.message.reply_text(f"📄 Дезинфекционный режим в подразделениях:\n{link}")
            mk = ReplyKeyboardMarkup(build_menu(MAIN_MENU), resize_keyboard=True)
            await update.message.reply_text("Вы в главном меню.", reply_markup=mk)
        elif text == "Микробиологический мониторинг":
            stack.append("mm_menu")
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Микробиологический мониторинг:", reply_markup=mk)
        elif text == "Аварийные ситуации":
            stack.append("as_menu")
            mk = ReplyKeyboardMarkup(build_menu(AS_MENU), resize_keyboard=True)
            await update.message.reply_text("Аварийные ситуации:", reply_markup=mk)
        elif text == "Особо опасные инфекции":
            stack.append("oOI_menu")
            mk = ReplyKeyboardMarkup(build_menu(OOI_MENU), resize_keyboard=True)
            await update.message.reply_text("Особо опасные инфекции:", reply_markup=mk)
        elif text == "Мероприятия при выявлении инфекционного больного":
            stack.append("infect_menu")
            mk = ReplyKeyboardMarkup(build_menu(INFECT_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип инфекции:", reply_markup=mk)
        else:
            await update.message.reply_text("Раздел временно недоступен.")
        return

    # === ИСМП меню ===
    if stack[-1] == "ism_menu":
        if text == "Комиссия по профилактике ИСМП":
            msg = (
                "Председатель:\n"
                "• Заместитель главного врача по медицинской части - Малышкина Ольга Александровна;\n\n"
                "Секретарь:\n"
                "• Врач-эпидемиолог – Чигиринов Иван Александрович;\n\n"
                "Члены комиссии:\n"
                "• Заместитель главного врача по клинико-экспертной работе – Москалева Марина Александровна;\n"
                "• Заведующий отделением анестезиологии и реанимации с палатой РИТ- врач-анестезиолог-реаниматолог – Теймуров Тимур Юсыфович;\n"
                "• Заведующий хирургическим отделением-врач-хирург – Булатов Алексей Юрьевич;\n"
                "• Заведующий отделением стационара-врач-кардиолог – Хомяченко Ольга Ивановна;\n"
                "• Заведующий амбулаторно-поликлиническим отделением-врач-терапевт – Оразмагомедова Эльвира Вячеславовна;\n"
                "• Заведующий клинико-диагностической лабораторией – Мерко Максим Михайлович;\n"
                "• Заведующий эндоскопическим отделением-врач-эндоскопист – Сигаева Юлия Сергеевна;\n"
                "• Клинический фармаколог – Борисов Алексей Михайлович;\n"
                "• Заведующий аптекой - Ремпенинг Анастасия Андреевна;\n"
                "• Главная медицинская сестра – Кочергина Лилия Алексеевна;"
            )
            mk = ReplyKeyboardMarkup(build_menu(ISM_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)

        elif text == "Критерии ИСМП":
            stack.append("criteria")
            mk = ReplyKeyboardMarkup(build_menu(CRITERIA_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите категорию:", reply_markup=mk)

        elif text == "Профилактика ИСМП у пациентов и персонала":
            msg = (
                "1. Соблюдение стандартных мер предосторожности \n"
                "• Гигиена рук – СОП-044-РН-СТ; СОП-045-РН-СТ;\n"
                "• Использование средств индивидуальной защиты (СИЗ) – СОП-13-23-РН-СТ;\n"
                "• Безопасное обращение с инъекционными препаратами- И-058-РН-СТ;\n"
                "• Обработка поверхностей и оборудования – И-025-РН-СТ;\n"
                "2. Стерилизация и дезинфекция медицинских изделий- И-018-РН-СТ; И-РН-СТ-2023-11-1;\n"
                "3. Профилактика инфекций при инвазивных процедурах – СОП-6,7,8,9,10,11,12-РН-СТ;\n"
                "4. Антибиотикопрофилактика и рациональное использование антибиотиков- И-065-РН-СТ; И-060-РН-СТ;\n"
                "5. Изоляция и контроль за источниками инфекции – СОП-14-23-РН-СТ;\n"
                "6. Вакцинация медицинского персонала – И-039\n"
                "• Обязательная вакцинация против:\n"
                "• Гепатита B (при отсутствии сведений о вакцинации/сведений о наличии антител)\n"
                "• Корь, краснуха, паротит (при отсутствии сведений о вакцинации/сведений о наличии антител)\n"
                "• Грипп (ежегодно)\n"
                "• Дифтерия, столбняк, коклюш (1 раз в 10 лет)\n"
                "7. Организационные меры – И-022-РН-СТ"
            )
            mk = ReplyKeyboardMarkup(build_menu(ISM_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)

        elif text == "Аббревиатуры":
            msg = (
                "Перечень основных аббревиатур:\n"
                "• ИАИ - имплантат-ассоциированная инфекция \n"
                "• ИВЛ – искусственная вентиляция легких \n"
                "• ИМП - инфекция мочевыводящих путей \n"
                "• ИДП - инфекции дыхательных путей\n"
                "• ИСМП - инфекция (инфекционная болезнь), связанная с оказанием медицинской помощи\n"
                "• ИОХВ - инфекция области хирургического вмешательства \n"
                "• КАИК - катетер-ассоциированная инфекция кровотока \n"
                "• ПАИ - перелом-ассоциированная инфекция \n"
                "• ППИ - перипротезная инфекция \n"
                "• ПВК- периферический венозный катетер \n"
                "• СОС – стандартное определение случая \n"
                "• ЦВК – центральный венозный катетер"
            )
            mk = ReplyKeyboardMarkup(build_menu(ISM_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)

        elif text == "Стандартные определения случаев ИСМП":
            stack.append("sos")
            mk = ReplyKeyboardMarkup(build_menu(SOS_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип инфекции:", reply_markup=mk)

        elif text == "Программа профилактики ИСМП":
            await update.message.reply_text(f"📄 Программа профилактики ИСМП:\n{PROGRAM_LINK}")
            mk = ReplyKeyboardMarkup(build_menu(ISM_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите пункт в ИСМП:", reply_markup=mk)

        elif text == "Выявленные ИСМП":
            stack.append("detected")
            mk = ReplyKeyboardMarkup(build_menu(DETECTED_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите подразделение:", reply_markup=mk)

        else:
            mk = ReplyKeyboardMarkup(build_menu(ISM_MENU), resize_keyboard=True)
            await update.message.reply_text("Пункт не найден.", reply_markup=mk)
        return

    # === Критерии ИСМП ===
    if stack[-1] == "criteria":
        if text == "У сотрудников":
            msg = (
                "1. клинически распознаваемое, в том числе с учетом результатов лабораторных исследований; \n"
                "2. возникло у работника при выполнении трудовых обязанностей в результате профессиональной деятельности; \n"
                "3. связано с оказанием медицинской помощи; \n"
                "4. не выявлено у работника при поступлении на работу в данную медицинскую организацию, в том числе в инкубационном периоде; \n"
                "5. не является обострением хронического инфекционного заболевания, имевшегося у работника до начала работы."
            )
            mk = ReplyKeyboardMarkup(build_menu(CRITERIA_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "У пациентов":
            msg = (
                "1. клинически распознаваемое, в том числе с учетом результатов лабораторных исследований; \n"
                "2. возникло в результате поступления пациента в медицинскую организацию или обращения за оказанием медицинской помощи вне зависимости от времени появления симптомов заболеваний; \n"
                "3. непосредственная связь их возникновения с оказанием медицинской помощи (лечением, диагностическими исследованиями, иммунизацией и пр.);\n"
                "4. отсутствовало у пациента при госпитализации или обращении за оказанием медицинской помощи в медицинскую организацию, в том числе в инкубационном периоде заболевания, кроме случаев инфекций, связанных с предшествующей госпитализацией или предшествующим обращением за оказанием медицинской помощи в медицинскую организацию; \n"
                "5. не является закономерным продолжением патологического процесса, имевшегося у пациента при госпитализации или обращении за оказанием медицинской помощи; \n"
                "6. не является обострением хронического инфекционного заболевания, имевшегося у пациента при госпитализации или обращении за оказанием медицинской помощи;"
            )
            mk = ReplyKeyboardMarkup(build_menu(CRITERIA_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(CRITERIA_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите категорию:", reply_markup=mk)
        return

    # === Выявленные ИСМП ===
    if stack[-1] == "detected":
        if text == "Стационар + Хирургическое отделение":
            msg = (
                "В период с 01.12.2025 по 01.11.2025  в отделении стационара + хирургическом отделении был выявлен 61 пациент с признаками инфекционного заболевания. Из них – к критериям ИСМП подходило – 12 пациентов (из них 12 – «ЗАНОСЫ»)\n"
                f"Ознакомиться с протоколами эпидрасследований можно по ссылке:\n{PROTOCOLS_LINK}"
            )
            mk = ReplyKeyboardMarkup(build_menu(DETECTED_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "ОАРИТ":
            msg = (
                "В период с 01.12.2025 по 01.11.2025  в ОАРИТ было выявлен 26 пациентов с признаками инфекционного заболевания. Из них – к критериям ИСМП подходило – 6 пациентов (из них 5 – «ЗАНОСЫ»; 1 – «СОБСТВЕННОЕ»\n"
                f"Ознакомиться с протоколами эпидрасследований можно по ссылке:\n{PROTOCOLS_LINK}"
            )
            mk = ReplyKeyboardMarkup(build_menu(DETECTED_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "Обобщенные данные":
            await update.message.reply_photo(photo=CHART_IMAGE_URL)
            mk = ReplyKeyboardMarkup(build_menu(DETECTED_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите подразделение:", reply_markup=mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(DETECTED_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите подразделение:", reply_markup=mk)
        return

    # === Стандартные определения случаев ИСМП ===
    if stack[-1] == "sos":
        sos_map = {
            "ИАИ - имплантат-ассоциированная инфекция": ("ИАИ:", IAI_MENU, "iai"),
            "ИДП – инфекции дыхательных путей": ("ИДП:", IDD_MENU, "idd"),
            "ИМП - инфекция мочевыводящих путей": ("ИМП:", IMP_MENU, "imp"),
            "ИОХВ - инфекция области хирургического вмешательства": ("ИОХВ:", IOHV_MENU, "iohv"),
            "ИК - инфекции кровотока": ("Инфекции кровотока:", IK_MENU, "ik"),
        }
        if text in sos_map:
            msg, menu_btns, next_state = sos_map[text]
            stack.append(next_state)
            mk = ReplyKeyboardMarkup(build_menu(menu_btns), resize_keyboard=True)
            await update.message.reply_text(msg, reply_markup=mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(SOS_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип инфекции:", reply_markup=mk)
        return

    # === ИАИ ===
    if stack[-1] == "iai":
        iai_texts = {
            "Определение ИАИ": (
                "Имплантат-ассоциированная инфекция- острая или хроническая инфекция области хирургического вмешательства, развившаяся после имплантации.\n"
                "Перипротезная инфекция- острая или хроническая инфекция области хирургического вмешательства, развившаяся после имплантации эндопротеза, представляет собой частный случай имплантатассоциированной инфекции.\n"
                "Перелом-ассоциированная инфекция- острый или хронический инфекционный процесс в области перелома костей, элементов остеосинтезирующего устройства (пластина, стержень, аппарат внешней фиксации и т.д.) и окружающих тканей"
            ),
            "Признаки ИАИ": (
                "1) Пациент имеет, по крайней мере, один из следующих признаков:\n"
                " свищевой ход, идущий в полость сустава;\n"
                " гнойное отделяемое из дренажа, установленного в полость протезированного сустава;\n"
                " выделение микроорганизмов из жидкости или ткани, полученной асептически из области протезированного сустава любым методом;\n"
                " дополнительно для перипротезной инфекции (ППИ) - количество лейкоцитов > 3000/мкл и/или доля нейтрофильных гранулоцитов > 70% в суставном аспирате при цитологическом исследовании.\n"
                "2) И\n"
                "при осмотре, во время повторной операции, при микробиологическом, гистопатологическом или рентгенологическом исследовании (фистулографии) обнаружение признаков инфекции, вовлекающей область протезированного сустава или вовлекающей область перелома и/или остеосинтеза"
            ),
            "Факторы риска развития ППИ": (
                "• Сахарный диабет\n"
                "• Ожирение или истощение\n"
                "• Цирроз печени\n"
                "• Алкоголизм\n"
                "• Наркомания\n"
                "• Хроническая почечная недостаточность (ХПН)\n"
                "• Иммуносупрессия (терапия глюкокортикоидами, цитостатиками, ВИЧ)\n"
                "• Спленэктомия\n"
                "• Ревматические заболевания\n"
                "• Предшествующие операции (инвазивные манипуляции) в области сустава\n"
                "• Наличие удалённых очагов инфекции"
            ),
            "Типы ППИ": (
                "I - Острая послеоперационная (менее 4 недель)\n"
                "II  - Поздняя хроническая (от 4 недель до одного года)\n"
                "III - Острая гематогенная/отсроченная (через год и более)\n"
                "IV - Положительная интраоперационная культура (положительные посевы в 2–5 интраоперационных образцах тканей)"
            ),
            "Вероятные пути инфицирования ППИ": (
                "I и II – Периоперационный путь (возбудители - S. Аureus; Streptococcus sp.; Enterococcus sp.; Коагулазонегативные стафилококки)\n"
                "III – Гематогенный путь (возбудители - S. Аureus; Streptococcus sp.; P. Acnes; E. Сoli)\n"
                "IV -  Гематогенный и периоперационный путь (возбудители - Коагулазонегативные стафилококки)"
            ),
            "Действия в случае выявления ИАИ": (
                "1) Лечащий врач информирует руководителя своего структурного подразделения и врача-эпидемиолога о подозрении на указанную инфекцию посредством электронного письма с указанием следующих критериев:\n"
                "• Ф.И.О. пациента, дата рождения, номер ЭМК;\n"
                "• дата появления симптомов, указывающих на ИОХВ;\n"
                "• возникшие симптомы.\n"
                "2) взять биологический материал для проведения лабораторного исследования у пациента из потенциального источника инфекции (до начала приема антимикробных препаратов), с целью подтверждения или опровержения диагноза инфекционного заболевания, а также этиологической расшифровки возбудителя (при наличии) инфекционного заболевания;\n"
                "3) при необходимости обеспечить перевод в специализированный стационар, согласно маршрутизации;\n"
                "4) выявить круг контактных лиц (список предоставить врачу эпидемиологу) из числа пациентов и персонала;\n"
                "5) провести экстренную профилактику контактных с клиническими проявлениями инфекционного заболевания (при необходимости);"
            ),
        }
        if text in iai_texts:
            mk = ReplyKeyboardMarkup(build_menu(IAI_MENU), resize_keyboard=True)
            await reply_safe(update, iai_texts[text], mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(IAI_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите пункт ИАИ:", reply_markup=mk)
        return

    # === ИДП ===
    if stack[-1] == "idd":
        if text == "Определение ИОД":
            stack.append("idd_def")
            mk = ReplyKeyboardMarkup(build_menu(IDD_DEF_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип:", reply_markup=mk)
        elif text == "Критерии ИОД":
            stack.append("idd_crit")
            mk = ReplyKeyboardMarkup(build_menu(IDD_CRIT_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип пневмонии:", reply_markup=mk)
        elif text == "Действия персонала при подозрении на ИОД":
            msg = (
                "Лечащий врач при подозрении на ИОД должен:\n"
                "1) незамедлительно изолировать пациента в отдельную палату (палату-изолятор);\n"
                "2) информировать руководителя своего структурного подразделения и врача-эпидемиолога о подозрении на указанную инфекцию устно (по телефону) и электронного письма с указанием следующих критериев:\n"
                "• Ф.И.О. пациента, дата рождения, номер ЭМК;\n"
                "• дата постановки инвазивного устройства;\n"
                "• дата смены устройства/комплектующих, его промывки;\n"
                "• дата санации;\n"
                "• дата появления симптомов, указывающих на ИОД;\n"
                "• возникшие симптомы;\n"
                "• приложить инструкцию к используемому оборудованию.\n\n"
                "3) взять биологический материал для проведения лабораторного исследования у пациента из потенциального источника инфекции (до начала приема антимикробных препаратов), с целью подтверждения или опровержения диагноза инфекционного заболевания, а также этиологической расшифровки возбудителя (при наличии) инфекционного заболевания;\n"
                "4) при необходимости обеспечить перевод в специализированный стационар, согласно маршрутизации;\n"
                "5) выявить круг контактных лиц (список предоставить врачу эпидемиологу) из числа пациентов и персонала;\n"
                "6) провести экстренную профилактику контактных с клиническими проявлениями инфекционного заболевания (при необходимости);"
            )
            mk = ReplyKeyboardMarkup(build_menu(IDD_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(IDD_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите действие:", reply_markup=mk)
        return

    if stack[-1] == "idd_def":
        def_map = {
            "Пневмония, связанная с оказанием медицинской помощи": (
                "Пневмония, связанная с оказанием медицинской помощи - пневмония, развившаяся через 48 ч и более после поступления в медицинскую организацию или обращения за оказанием медицинской помощи, в т.ч. госпитализации в стационар (внутрибольничная пневмония). Делятся на :\n"
                "1) пневмонии, связанные с оказанием медицинской помощи, без микробиологического подтверждения; \n"
                "2) пневмонии, связанные с оказанием медицинской помощи, микробиологически подтвержденные. \n"
                "3) также выделяют пневмонии, связанные с искусственной вентиляцией легких (ИВЛ ассоциированные пневмонии), которые также могут быть микробиологически подтвержденные или без микробиологического подтверждения."
            ),
            "Трахеобронхиальные инфекции (бронхит, трахеобронхит, бронхиолит, трахеит без признаков пневмонии)": (
                "Трахеобронхиальные инфекции (бронхит, трахеобронхит, бронхиолит, трахеит без признаков пневмонии) - это воспалительные процессы в трахее и бронхах, которые чаще всего развиваются по причине вторжения вирусной или бактериальной инфекции.\n"
                "Трахеобронхиальные инфекции должны удовлетворять ХОТЯ БЫ ОДНОМУ из следующих критериев:\n"
                "Критерий 1: \n"
                " У пациента НЕ имеется клинических или рентгенологических признаков пневмонии\n"
                "И \n"
                " У пациента имеются ПО КРАЙНЕЙ МЕРЕ ДВА из следующих признаков или симптомов без другой установленной причины: лихорадка (> 37,5 С), кашель, вновь появившаяся гнойная мокрота или увеличение ее продукции, хрипы, тяжелое дыхание \n"
                "И \n"
                " хотя бы ОДНО из перечисленных обстоятельств:\n"
                "1. Выделение возбудителя из клинического материала, полученного путем глубокой трахеальной аспирации или при бронхоскопии \n"
                "2. Обнаружение антигена в секретах дыхательных путей \n"
                "Критерий 2: \n"
                " У пациента имеются ПО КРАЙНЕЙ МЕРЕ ДВА из следующих признаков или симптомов без другой установленной причины: лихорадка (> 37,5 С), кашель, вновь появившаяся гнойная мокрота или увеличение ее продукции, хрипы, тяжелое дыхание, респираторный дистресс, апноэ или брадикардия \n"
                "И \n"
                " хотя бы ОДНО из перечисленных обстоятельств: \n"
                "1. Выделение возбудителя из клинического материала, полученного путем глубокой трахеальной аспирации или при бронхоскопии \n"
                "2. Обнаружение антигена в секретах дыхательных путей \n"
                "Диагностически значимый титр антител класса IgM или четырехкратное возрастание в парных образцах сыворотки уровня антител класса IgG к возбудителю"
            ),
            "Прочие инфекции нижних дыхательных путей": (
                "Прочие инфекции нижних дыхательных путей должны удовлетворять ХОТЯ БЫ ОДНОМУ из следующих критериев: \n"
                "Критерий 1: \n"
                "В мазке или культуре из легочной ткани или жидкости, включая плевральную жидкость, обнаружены микроорганизмы \n"
                "Критерий 2: \n"
                "У пациента имеется абсцесс легкого или эмпиема, обнаруженные в ходе хирургической операции или при гистопатологическом исследовании \n"
                "Критерий 3: \n"
                "У пациента имеются рентгенологические признаки абсцесса легкого."
            )
        }
        mk = ReplyKeyboardMarkup(build_menu(IDD_DEF_MENU), resize_keyboard=True)
        await reply_safe(update, def_map.get(text, "Текст не найден."), mk)
        return

    if stack[-1] == "idd_crit":
        crit_map = {
            "пневмонии без микробиологического подтверждения": (
                " развилась через 48 часов и более после поступления в медицинскую организацию или обращения за оказанием медицинской помощи; \n"
                " не является закономерным развитием основного заболевания. \n"
                "И\n\n"
                "Наличие признаков пневмонии при рентгенологическом исследовании или КТ- 25 сканировании грудной клетки (инфильтрат, консолидация, кавитация) \n"
                "И\n\n"
                "Не менее двух из следующих симптомов:\n"
                "1) лихорадка > 38,0°C без других причин; \n"
                "2) лейкопения (< 4 000 лейкоцитов/мм3 ) или лейкоцитоз (≥ 12 000 лейкоцитов/мм3 ). \n"
                "3) вновь появившаяся гнойная мокрота или изменение характера мокроты (цвет, запах, количество, консистенция); \n"
                "4) кашель, диспноэ или тахипноэ;\n"
                "5) подозрение при аускультации (хрипы, крепитация или звуки бронхиального дыхания); \n"
                "6) ухудшение газообмена (например, O2- десатурация или увеличение потребности в кислороде или увеличение потребности в вентиляции).\n\n"
                "Примечания: \n"
                "** У пациентов с сопутствующими заболеваниями сердца или легких (респираторный дистресс-синдром, бронхо-легочная дисплазия, отек легких, хроническая обструктивная болезнь легких и др.) может понадобиться дополнительное рентгенологическое исследование или КТ-сканирование легких."
            ),
            "пневмонии микробиологически подтвержденные": (
                " развилась через 48 часов и более после поступления в медицинскую организацию или обращения за оказанием медицинской помощи; \n"
                " не является закономерным развитием основного заболевания. \n"
                "И \n\n"
                "1. Наличие признаков пневмонии при рентгенологическом исследовании или КТ-сканировании грудной клетки*, ** \n"
                "И \n\n"
                "2. Как минимум один из следующих симптомов: \n"
                " лихорадка > 38,0°C без других причин: \n"
                " лейкопения (< 4 000 лейкоцитов/мм3 ) или лейкоцитоз (≥ 12 000 лейкоцитов/мм3 ). \n"
                "И \n\n"
                "3. Один из следующих симптомов (или не менее двух, если имеется высев культуры из мокроты или положительный неколичественный посев материала из нижних отделов дыхательных путей): \n"
                " вновь появившаяся гнойная мокрота, или изменение характера мокроты (цвет, запах, 26 количество, консистенция);\n"
                " кашель, диспноэ или тахипноэ;\n"
                " подозрение при аускультации (хрипы, крепитация или звуки бронхиального дыхания);\n"
                " ухудшение газообмена (например, O2- десатурация или увеличение потребности в кислороде, или увеличение потребности в вентиляции). \n"
                "И \n\n"
                "4. Как минимум один из следующих результатов микробиологических исследований: \n"
                " положительный количественный высев из минимально контаминированных образцов нижних дыхательных путей с пороговой величиной: \n"
                "• при бронхо-альвеолярном лаваже (БАЛ) > 10*4 КОЕ/мл или при прямом микроскопическом исследовании ≥ 5% клеток, содержащих бактерии, расположенные внутриклеточно; \n"
                "• защищенной щеткой > 10*3 КОЕ/мл; \n"
                "• дистальной защищенной аспирацией > 10*3 КОЕ/мл.\n"
                " положительный количественный высев из возможно контаминированного образца с пороговой величиной: \n"
                "• из образца нижних дыхательных путей (например, эндотрахеальный аспират) 10*6 КОЕ/мл. \n"
                " положительный высев из крови, не связанный с инфекцией другой локализации; - положительный высев из аспирата при пункции легочного абсцесса; \n"
                " положительный результат исследования на присутствие респираторных вирусов (гриппа, РС-Вирусы, аденовирусы, коронавирусы, бокавирусы, метапневмовирусы и др.) или микроорганизмов (например, Legionella spp., Aspergillus spp., и другие): \n"
                "• обнаружение вирусных антигенов или антител в респираторном секрете (молекулярно-биологическими, иммунохроматографическими методами); \n"
                "• сероконверсия (например, вирус гриппа, Legionella spp., Mycoplasma spp.); \n"
                "• обнаружение антигенов в моче 27 (Legionella spp., S. Pneumoniae). - положительный высев из мокроты***\n\n"
                "Примечания:\n"
                "• * Инфильтрат, консолидация, кавитация \n"
                "• ** У пациентов с сопутствующими заболеваниями сердца или легких (респираторный дистресс-синдром, бронхо - легочная дисплазия, отек легких, хроническая обструктивная болезнь легких и др.) может понадобиться дополнительное рентгологическое исследование или КТ-сканирование легких \n"
                "• *** Данный критерий используется только в целях эпидемиологической диагностики."
            ),
            "ИВЛ-ассоциированные пневмонии": (
                "1. Пневмония определяется как связанная с искусственной вентиляцией легких (ИВЛ)*, если пневмония развилась не ранее, чем через 48 ч от момента начала проведения ИВЛ, при отсутствии признаков легочной инфекции на момент начала ИВЛ. ** \n"
                "И\n\n"
                "2. Должна соответствовать критериям пневмонии, связанной с оказанием медицинской помощи, без микробиологического подтверждения.\n"
                "ИЛИ \n\n"
                "3. Должна соответствовать критериям пневмонии, связанной с оказанием медицинской помощи, микробиологически подтвержденной. \n\n"
                "Примечания: \n"
                "* К ИВЛ относится респираторная поддержка, осуществляемая через трахеостому или эндотрахеальную интубацию. \n"
                "** Если ИВЛ была начата в день появления симптомов пневмонии без дополнительной информации о последовательности событий, пневмония не рассматривается как связанная с ИВЛ."
            )
        }
        mk = ReplyKeyboardMarkup(build_menu(IDD_CRIT_MENU), resize_keyboard=True)
        await reply_safe(update, crit_map.get(text, "Текст не найден."), mk)
        return

    # === ИМП ===
    if stack[-1] == "imp":
        if text == "Определение ИМП":
            msg = (
                "Инфекции мочевыделительных путей (ИМП) - это воспалительное заболевание, вызванное проникновением и размножением патогенных микроорганизмов в одном или нескольких отделах мочевыделительной системы. Их делят на \n"
                "- микробиологически подтвержденная симптоматическая ИМП;\n"
                "- не подтвержденная микробиологически симптоматическая ИМП. \n"
                "- также выделяют катетер-ассоциированную ИМП"
            )
            mk = ReplyKeyboardMarkup(build_menu(IMP_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "Виды и критерии ИМП":
            stack.append("imp_types")
            mk = ReplyKeyboardMarkup(build_menu(IMP_TYPES_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите вид ИМП:", reply_markup=mk)
        elif text == "Действия персонала при выявлении ИМП":
            msg = (
                "Лечащий врач при подозрении на ИМП должен:\n"
                "1) информировать руководителя своего структурного подразделения и врача-эпидемиолога о подозрении на указанную инфекцию устно (по телефону) и электронного письма с указанием следующих критериев:\n"
                "• Ф.И.О. пациента, номер ЭМК;\n"
                "• наличие в анамнезе хронического процесса\n"
                "• дата постановки инвазивного устройства;\n"
                "• дата смены устройства/комплектующих, его промывки;\n"
                "• дата санации;\n"
                "• дата появления симптомов, указывающих на ИМП;\n"
                "• возникшие симптомы;\n\n"
                "2) взять биологический материал для проведения лабораторного исследования у пациента из потенциального источника инфекции (до начала приема антимикробных препаратов), с целью подтверждения или опровержения диагноза инфекционного заболевания, а также этиологической расшифровки возбудителя (при наличии) инфекционного заболевания;\n"
                "3) при необходимости обеспечить перевод в специализированный стационар, согласно маршрутизации;\n"
                "4) выявить круг контактных лиц (список предоставить врачу эпидемиологу) из числа пациентов и персонала;\n"
                "5) провести экстренную профилактику контактных с клиническими проявлениями инфекционного заболевания (при необходимости);"
            )
            mk = ReplyKeyboardMarkup(build_menu(IMP_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(IMP_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите пункт ИМП:", reply_markup=mk)
        return

    if stack[-1] == "imp_types":
        imp_map = {
            "микробиологически подтвержденная симптоматическая ИМП": (
                "Должна соответствовать следующим критериям: \n"
                "1. Отсутствовать у пациента при поступлении, в том числе в инкубационном периоде. \n"
                "2. Не является закономерным развитием основного заболевания. \n"
                "И\n"
                "3. У пациента есть, по крайней мере, один из следующих симптомов без какой-либо другой установленной причины: \n"
                "- лихорадка (> 38°C); \n"
                "- резкие позывы к мочеиспусканию; \n"
                "- учащенное мочеиспускание; \n"
                "- дизурия или в болезненность надлобковой области.\n"
                "И \n"
                "4. По крайней мере, одно из следующих: \n"
                "- выделение урокультуры в количестве ≥ 105 КОЕ/мл мочи, не более, чем 2 видов микроорганизмов; \n"
                "- две повторно выделенные урокультуры одного и того же микроорганизма в количестве ≥ 102 КОЕ/мл из проб, взятых асептически; \n"
                "- одна урокультура микроорганизма в количестве ≤ 105 КОЕ/мл у пациента, получающего соответствующую антимикробную терапию."
            ),
            "не подтвержденная микробиологически симптоматическая ИМП": (
                "Должна соответствовать следующим критериям: \n"
                "- отсутствовать у пациента при поступлении, в том числе в инкубационном периоде; \n"
                "- не является закономерным развитием основного заболевания. \n"
                "И\n"
                "2. У пациента, по крайней мере, один из следующих симптомов без какой-либо другой установленной причины: \n"
                "- лихорадка (> 38°C); \n"
                "- резкие позывы к мочеиспусканию; \n"
                "- учащенное мочеиспускание; \n"
                "- дизурия или болезненность надлобковой области. \n"
                "И\n"
                "3. По крайней мере, одно из следующих: \n"
                "- пиурия с количеством лейкоцитов ≥ 10 в 1 мл или ≥ 3 лейкоцита в поле зрение при микроскопии с высоким разрешением образца нецентрифугированной мочи;\n"
                "- обнаружением микроорганизма при окраске по Граму образца нецентрифугированной мочи; \n"
                "- назначение врачом соответствующей антимикробной терапии. "
            ),
            "катетер-ассоциированная ИМП": (
                "Должна соответствовать следующим критериям: \n"
                "1.Катетер-ассоциированной ИМП считается инфекция, соответствующая стандартным определениям ИМП. \n"
                "И\n"
                "2.Возникшая у пациента не ранее, чем через 48 часов после постановки внутреннего мочевого катетера. \n"
                "ИЛИ\n"
                "3.Возникшая у пациента в течение 48 часов после удаления внутреннего мочевого катетера, стоявшего не менее 48 часов."
            )
        }
        mk = ReplyKeyboardMarkup(build_menu(IMP_TYPES_MENU), resize_keyboard=True)
        await reply_safe(update, imp_map.get(text, "Текст не найден."), mk)
        return

    # === ИОХВ ===
    if stack[-1] == "iohv":
        if text == "Определение ИОХВ":
            msg = (
                "Инфекция области хирургического вмешательства - инфекция хирургического разреза, органа или полости, возникающая в течение первых 30 дней послеоперационного периода (при наличии имплантата – до 90 дней). Делятся на:\n"
                "1) Поверхностная инфекция области хирургического вмешательства\n"
                "2) Глубокая инфекция области хирургического вмешательства\n"
                "3) Инфекция области хирургического вмешательства органа или полости"
            )
            mk = ReplyKeyboardMarkup(build_menu(IOHV_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "Виды и критерии ИОХВ":
            stack.append("iohv_types")
            mk = ReplyKeyboardMarkup(build_menu(IOHV_TYPES_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип ИОХВ:", reply_markup=mk)
        elif text == "Действия персонала при выявлении ИОХВ":
            msg = (
                "1) Информировать руководителя своего структурного подразделения и врача-эпидемиолога о подозрении на указанную инфекцию устно (по телефону) и электронного письма с указанием следующих критериев:\n"
                "• Ф.И.О. пациента, номер ЭМК;\n"
                "• наличие в анамнезе хронического процесса\n"
                "• дата постановки инвазивного устройства;\n"
                "• дата смены устройства/комплектующих, его промывки;\n"
                "• дата санации;\n"
                "• дата появления симптомов, указывающих на ИОХВ;\n"
                "• возникшие симптомы;\n"
                "2) взять биологический материал для проведения лабораторного исследования у пациента из потенциального источника инфекции (до начала приема антимикробных препаратов), с целью подтверждения или опровержения диагноза инфекционного заболевания, а также этиологической расшифровки возбудителя (при наличии) инфекционного заболевания;\n"
                "3) при необходимости обеспечить перевод в специализированный стационар, согласно маршрутизации;\n"
                "4) выявить круг контактных лиц (список предоставить врачу эпидемиологу) из числа пациентов и персонала;\n"
                "5) провести экстренную профилактику контактных с клиническими проявлениями инфекционного заболевания;"
            )
            mk = ReplyKeyboardMarkup(build_menu(IOHV_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(IOHV_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите пункт ИОХВ:", reply_markup=mk)
        return

    if stack[-1] == "iohv_types":
        iohv_map = {
            "Поверхностная инфекция области хирургического вмешательства": (
                "1) Критерии:\n"
                "• имеет связь с хирургическим вмешательством;\n"
                "• не является закономерным развитием основного заболевания;\n"
                "• включает только кожу и подкожную клетчатку;\n"
                "• срок развития инфекции не превышает 30 дней после любого оперативного вмешательства (первым днём считается день хирургического вмешательства).\n"
                "2) Признаки 1-й  группы:\n"
                "• гнойное отделяемое из раны;\n"
                "• выделение микроорганизмов из жидкости или ткани, полученной асептически из поверхностного разреза или подкожной клетчатки культуральным или другим методом;\n"
                "• хирург намеренно открывает рану, при этом микробиологическое исследование не проведено;\n"
                "3) Признаки 2 -й группы:\n"
                "• локальное воспаление (боль, гиперемия, отёк) с гипертермией за исключением тех случаев, когда посев отделяемого раны даёт отрицательный результат;\n"
                "• хирург или лечащий врач поставил диагноз инфекции области хирургического вмешательства."
            ),
            "Глубокая инфекция области хирургического вмешательства": (
                "1) Критерии:\n"
                "• имеет связь с хирургическим вмешательством;\n"
                "• не является закономерным развитием основного заболевания;\n"
                "• вовлекает в гнойный процесс глубокие мягкие ткани (фасциальный и мышечный слои);\n"
                "• срок развития инфекции не превышает 30 дней после любого оперативного вмешательства (первым днём считается день хирургического вмешательства), а при наличии имплантата – 90 дней.\n"
                "2) Признаки 1 группы:\n"
                "• гнойное отделяемое из глубины разреза, но не из органа (полости) в месте данного хирургического вмешательства;\n"
                "• спонтанное расхождение краёв раны или намеренное её открытие хирургом, когда у пациента имеются признаки инфекции и любым методом выделен микроорганизм;\n"
                "3) Признаки 2 группы:\n"
                "• лихорадка (> 38°C), локализованная боль, за исключением тех случаев, когда посев из раны даёт отрицательные результаты;\n"
                "• при осмотре, во время повторной операции, при гистопатологическом или лучевом методе исследования обнаружен абсцесс или иные признаки инфекции в области хирургического разреза."
            ),
            "Инфекция области хирургического вмешательства органа или полости": (
                "1) Критерии:\n"
                "• имеет связь с хирургическим вмешательством;\n"
                "• не является закономерным развитием основного заболевания;\n"
                "• срок развития инфекции не превышает 30 дней после любого оперативного вмешательства (первым днём считается день хирургического вмешательства), а при наличии имплантата – 90 дней;\n"
                "• вовлекает в гнойный процесс органы (полости), которые были открыты или затронуты во время операции, исключая кожу, подкожную клетчатку и глубокие мягкие ткани (фасциальный и мышечный слои).\n"
                "2) Признаки 1 группы:\n"
                "• гнойное отделяемое из дренажа, установленного в органе (полости);\n"
                "• свищевой ход, связанный с имплантом;\n"
                "• гнойное отделяемое в области установки импланта;\n"
                "• выделение микроорганизмов из жидкости или ткани, полученной асептически из органа (полости) любым методом.\n"
                "3) Признаки 2 группы:\n"
                "• при непосредственном осмотре, во время повторной операции, при гистопатологическом или лучевом методе исследования обнаружение абсцесса или иных признаков инфекции, вовлекающей орган или полость."
            )
        }
        mk = ReplyKeyboardMarkup(build_menu(IOHV_TYPES_MENU), resize_keyboard=True)
        await reply_safe(update, iohv_map.get(text, "Текст не найден."), mk)
        return

    # === ИК ===
    if stack[-1] == "ik":
        if text == "Определения ИК":
            msg = (
                "Инфекция кровотока - инфекция любой этиологии, при которой положительный посев крови связан с клиническими признаками и симптомами инфекции.\n"
                "Катетер ассоциированная инфекция кровотока - первичная инфекция кровотока у пациента, которая развилась через 48 часов и более после введения в кровеносное русло катетера и нет связи с другим источником инфекции.\n"
                "Инфекция, связанная с сосудистым катетером – это местная инфекция в ране входного отверстия катетера и инфекция кровотока у пациента с сосудистым катетером при микробиологическом подтверждении роли сосудистого катетера как фактора передачи возбудителя инфекции."
            )
            mk = ReplyKeyboardMarkup(build_menu(IK_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "Виды и критерии ИК":
            stack.append("ik_types")
            mk = ReplyKeyboardMarkup(build_menu(IK_TYPES_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип инфекции кровотока:", reply_markup=mk)
        elif text == "Действия персонала при выявлении ИК":
            msg = (
                "1) Лечащий врач информирует руководителя своего структурного подразделения и врачей-эпидемиологов о подозрении на указанную инфекцию посредством электронного письма с указанием следующих критериев:\n"
                "• Ф.И.О. пациента, дата рождения, номер ЭМК;\n"
                "• дата постановки катетера, его локализация;\n"
                "• дата смены катетера, его промывки;\n"
                "• название катетера;\n"
                "• дата появления симптомов, указывающих на КАИК;\n"
                "• возникшие симптомы;\n"
                "2) взять биологический материал для проведения лабораторного исследования у пациента из потенциального источника инфекции (до начала приема антимикробных препаратов), с целью подтверждения или опровержения диагноза инфекционного заболевания, а также этиологической расшифровки возбудителя (при наличии) инфекционного заболевания;\n"
                "3) при необходимости обеспечить перевод в специализированный стационар, согласно маршрутизации;\n"
                "4) выявить круг контактных лиц (список предоставить врачу эпидемиологу) из числа пациентов и персонала;\n"
                "5) провести экстренную профилактику контактных с клиническими проявлениями инфекционного заболевания (при необходимости);"
            )
            mk = ReplyKeyboardMarkup(build_menu(IK_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(IK_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите пункт ИК:", reply_markup=mk)
        return

    if stack[-1] == "ik_types":
        ik_map = {
            "Лабораторно подтвержденная инфекция кровотока": (
                "Должна соответствовать следующим критериям: \n"
                "1. Один положительный высев из крови признанного (значимого) патогена*. \n"
                "ИЛИ \n"
                "2.1. Пациент имеет хотя бы один из следующих симптомов: лихорадка (> 38°C), озноб или гипотензия. \n"
                "И \n"
                "2.2. Пациент имеет из крови два положительных высева микроорганизма, входящего в состав нормальной микрофлоры кожи** (из двух отдельно взятых посевов в течение 48 часов). \n\n"
                "Примечания: В целях эпидемиологического надзора в данном стандартном определении случая принято следующее: \n"
                "- * к патогенным микроорганизмам относятся: Staphylococcus aureus, Грам отрицательные бактерии (Pseudomonas spp, Acinetobacter spp, Achromobacter spp, Stenotrophomonas maltophilia и др.), дрожжеподобные/ плесневые грибы; \n"
                "- ** к нормальной микрофлоре кожи отнесены: коагулазоотрицательные стафилококки, Micrococcus spp., Propionibacterium acne, Bacillus spp., Corynebacterium spp. и др."
            ),
            "Катетер-ассоциированная инфекция кровотока (КАИК)": (
                "Должна соответствовать следующим критериям: \n"
                "1. Катетер установлен более 48 часов назад до развития инфекции кровотока. \n"
                "И \n"
                "2. Источником микроорганизма не является другой очаг инфекции. \n"
                "И: \n"
                "Пациент имеет, по крайней мере, один из следующих признаков: \n"
                "3. Один положительный высев из крови признанного патогена.\n"
                "ИЛИ\n"
                "4.1. Пациент имеет хотя бы один из следующих симптомов: лихорадка (> 38°C), озноб или гипотензия. \n"
                "И \n"
                "4.2. Два положительных высева микроорганизма, входящего в состав нормальной микрофлоры кожи (из двух отдельно взятых посевов в течение 48 часов), из крови пациента. \n\n"
                "Примечания: Диагноз КАИК будет окончательным, в случае если у пациента с сосудистым катетером: \n"
                "• наблюдались клинико-лабораторные признаки инфекции кровотока; \n"
                "• не были найдены другие очаги инфекции;\n"
                "• отсутствовала клиническая возможность удалить сосудистый катетер и провести его микробиологическое исследование с целью доказательства этиологически значимого контаминирования катетера микроорганизмами (т.е. не удалось идентифицировать сосудистый катетер как фактор передачи инфекции). \n"
                "Для подтверждения роли катетера, как конечного фактора передачи инфекции и его этиологической значимости в патогенезе инфекции кровотока требуется микробиологическое исследование дистального сегмента удаленного сосудистого катетера. Наиболее достоверными являются количественный и полуколичественный методы посева сегмента катетера. \n"
                "В случае микробиологического подтверждения роли сосудистого катетера, как фактора передачи, местная инфекция в ране входного отверстия катетера и инфекция кровотока у пациента с сосудистым катетером будут отнесены к группе инфекций, связанных с сосудистым катетером."
            ),
            "ИК связанная с ЦВК (без положительного высева из крови)": (
                "Должна соответствовать следующим критериям: \n"
                "1. Катетер установлен более 48 часов назад.\n"
                "И\n"
                "2. Гной/воспаление в ране входного отверстия катетера. \n"
                "И \n"
                "3. Количественный посев ЦВК ≥ 10*3 КОЕ/мл или полуколичественный посев ЦВК > 15 КОЕ. Примечание: Местная инфекция, связанная с ЦВК, включает и туннельную/карманную инфекцию туннелированного/ тотально имплантированного катетера."
            ),
            "ИК ПВК (без положительного высева из крови)": (
                "Должна соответствовать следующим критериям: \n"
                "1. Катетер установлен более 48 часов назад.\n"
                "И\n"
                "2. Гной/воспаление в ране входного отверстия катетера. \n"
                "И \n"
                "3. Количественный посев ПВК ≥ 10*3 КОЕ/мл или полуколичественный посев ПВК > 15 КОЕ. "
            ),
            "Генерализованная ИК, связанная с ЦВК (без положительного высева из крови)": (
                "Должна соответствовать следующим критериям:\n"
                "1. Катетер установлен более 48 часов назад. \n"
                "И\n"
                "2. Количественный посев ЦВК ≥ 10*3 КОЕ/мл или полуколичественный посев ЦВК > 15 КОЕ. \n"
                "И\n"
                "3. Улучшение клинической картины в течение 48 часов после удаления катетера."
            ),
            "Генерализованная ИК, связанная с ПВК (без положительного высева из крови)": (
                "Должна соответствовать следующим критериям: \n"
                "1. Катетер установлен более 48 часов назад.\n"
                "И\n"
                "2. Количественный посев ПВК ≥ 10*3 КОЕ/мл или полуколичественный посев ПВК > 15 КОЕ. \n"
                "И\n"
                "3. улучшение клинической картины в течение 48 часов после удаления катетера."
            ),
            "Микробиологически подтвержденная инфекция кровотока, связанная с ЦВК": (
                "Должна соответствовать следующим критериям: \n"
                "1. Инфекция кровотока, возникшая за 48 часов до или после удаления катетера, \n"
                "И\n"
                "2. Пациент имеет хотя бы один из следующих признаков (в зависимости от клинической возможности удаления катетера и применяемого метода микробиологического исследования):\n"
                "• выделение одного и того же микроорганизма из крови и из удаленного катетера количественным методом посева ЦВК 10*3 КОЕ/мл или полуколичественным методом посева ЦВК > 15 КОЕ;\n"
                "• количественное соотношение микробных клеток одного и того же микроорганизма в образцах крови, взятых одновременно из ЦВК и из периферической вены > 5*;\n"
                "• дифференциальная задержка положительного результата посева крови: положительный результат пробы крови из ЦВК идентифицируется ранее на два или более часа в сравнении с положительным результатом пробы крови из периферической вены (образцы крови берутся одновременно)*. \n"
                "И\n"
                "3. Выделение одного и того же микроорганизма из крови и из отделяемого из раны входного отверстия катетера (в случае его наличия и при подозрении на местную инфекцию)\n\n"
                "Примечание: * Данные методы микробиологического подтверждения инфекции кровотока, связанной с ЦВК, относятся к методам, не требующим удаления катетера. Применяются в случае, если нет клинической возможности удалить подозрительный сосудистый катетер и исследовать его микробиологическими методами."
            ),
            "Микробиологически подтвержденная инфекция кровотока, связанная с ПВК": (
                "Должна соответствовать следующим критериям: \n"
                "1. Инфекция кровотока, возникшая за 48 часов до или после удаления катетера.\n"
                "И \n"
                "2. Выделение одного и того же микроорганизма из крови и из катетера количественным методом посева ЦВК 10*3 КОЕ/мл или полуколичественным методом посева ЦВК > 15 КОЕ. \n"
                "ИЛИ \n"
                "3. Выделение одного и того же микроорганизма из крови и из отделяемого из раны входного отверстия катетера (в случаи его наличия)."
            )
        }
        mk = ReplyKeyboardMarkup(build_menu(IK_TYPES_MENU), resize_keyboard=True)


# === Мероприятия при выявлении инфекционного больного (текстовые ссылки) ===
    if stack[-1] == "infect_menu":
        link_map = {
        "Первичные противоэпидемические мероприятия": "https://disk.yandex.ru/i/s52xCqt_eZicCQ",
        "Действия в случае выявления ВИЧ-инфекции у пациента": "https://disk.yandex.ru/i/MRp6KIj7zEhPuw",
        "Действия при выявлении гепатита В и С у пациента": "https://disk.yandex.ru/i/8RQRRNAMmrxqfA",
        "Действия в случае выявления туберкулеза у пациента": "https://disk.yandex.ru/i/qK-O_HfyAKIWgw",
        "Действия в случае выявления сальмонеллеза у пациента": "https://disk.yandex.ru/i/xzvSV4e0LIK7OQ",
        "Действия в случае выявления Гепатита А и Е": "https://disk.yandex.ru/i/zeZC8_IBi-LlSw",
        "Противоэпидемические мероприятия против Грипп, ОРВИ, Covid-19": "https://disk.yandex.ru/i/kaD1_OMYU53XTA",
        "Действия при выявлении кори, краснухи, эпидпаротита у пациента": "https://disk.yandex.ru/i/ekx5SOiq7B48ng",
        "Действия при выявлении коклюша у пациента": "https://disk.yandex.ru/i/OiWzoU_SFpcFaw",
        "Действия при выявлении дифтерии у пациента": "https://disk.yandex.ru/i/yDhq-ppTSgbC5Q",
        "Действия в случае выявления менингококковой инфекции": "https://disk.yandex.ru/i/J1z98NuOvt23mg",
        "Действия в случае выявления пневмонии у пациента": "https://disk.yandex.ru/i/lxszApbrFfSjSg",
        "Действия при выявлении стрептококковой инфекции группы А у пациента": "https://disk.yandex.ru/i/4Zkp_KK3GJr6Fw",
        "Действия в случае выявления легионеллеза": "https://disk.yandex.ru/i/VMtdtyH1GlGgvQ",
        "Действия при выявлении гнойно-воспалительных заболеваний": "https://disk.yandex.ru/i/nCHBXX63VtqbeA",
    }
    if text in link_map:
        await update.message.reply_text(f"📎 {text}:\n{link_map[text]}")
    else:
        await update.message.reply_text("Пункт не найден.")

    # Возврат в подменю
    mk = ReplyKeyboardMarkup(build_menu(INFECT_MENU), resize_keyboard=True)
    await update.message.reply_text("Выберите тип инфекции:", reply_markup=mk)
    return

    # === По умолчанию ===
    await update.message.reply_text("Пожалуйста, используйте кнопки меню.")


    # === Микробиологический мониторинг ===
    if stack[-1] == "mm_menu":
        if text == "Группы микроорганизмов (пациенты)":
            stack.append("mm_group_mo")
            mk = ReplyKeyboardMarkup(build_menu(MM_GROUP_MO_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите год:", reply_markup=mk)
        elif text == "Вид микроорганизмов (пациенты)":
            stack.append("mm_species_mo")
            mk = ReplyKeyboardMarkup(build_menu(MM_SPECIES_MO_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите год:", reply_markup=mk)
        elif text == "Профили резистентности пациентов":
            stack.append("mm_resist")
            mk = ReplyKeyboardMarkup(build_menu(MM_RESIST_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите год:", reply_markup=mk)
        elif text == "Нормативные документы":
            msg = (
                "1) СанПиН 3.3686-21 «Профилактика инфекционных заболеваний»\n"
                "2) МР 3.1.0346-24 «Организация и проведение микробиологического мониторинга в медицинских организациях»\n"
                "3) И-ПЛНА-РН-СТ-2024-11-1 «Инструкция о порядке реализации системы микробиологического мониторинга»\n"
                "4) И-022-РН-СТ «О порядке реализации системы эпидемиологической безопасности»"
            )
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "Производственный контроль":
            await update.message.reply_document(document="https://disk.yandex.ru/i/g59PyXf3ywDUNA")
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Микробиологический мониторинг:", reply_markup=mk)
        elif text == "Перечень биоматериала для планового микробиологического исследования":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/jIiEJ_eI0OGuLw")
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Микробиологический мониторинг:", reply_markup=mk)
        elif text == "Годовой отчет ММ":
            await update.message.reply_document(document="https://disk.yandex.ru/i/FNVqv_PrXK13vw")
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Микробиологический мониторинг:", reply_markup=mk)
        elif text == "Оценка эффективности дезинфицирующих средств":
            msg = (
                "На 11.08.2025 года была оценена эффективность дезинфицирующего раствора «Актосед плюс» 0,5% в отношении Ps. Aeruginosa, Ac. Baumanii, Kl.pneumoniae (панрезистентная).\n"
                "Результат- Штаммы Ps. Aeruginosa, Ac. Baumanii, Kl.pneumoniae (панрезистентная) чувствительны к действию дезсредства «Актосед плюс» на 100%"
            )
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await reply_safe(update, msg, mk)
        elif text == "Частота ESCAPE-патогенов":
            stack.append("mm_escape")
            mk = ReplyKeyboardMarkup(build_menu(MM_ESCAPE_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите год:", reply_markup=mk)
        elif text == "Коэффициент микробного разнообразия":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/WBqa9WOXvGbZJg")
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Микробиологический мониторинг:", reply_markup=mk)
        elif text == "Пациенты с полирезистентными штаммами":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/bxAj63ypzyTvQg")
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Микробиологический мониторинг:", reply_markup=mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(MM_MENU), resize_keyboard=True)
            await update.message.reply_text("Пункт не найден.", reply_markup=mk)
        return

    # === Группы МО ===
    if stack[-1] == "mm_group_mo":
        if text == "2024 год группы МО":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/-M7amcn-2X3Elg")
        elif text == "2025 год группы МО":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/CaQLYpnF_0LNYg")
        mk = ReplyKeyboardMarkup(build_menu(MM_GROUP_MO_MENU), resize_keyboard=True)
        await update.message.reply_text("Выберите год:", reply_markup=mk)
        return

    # === Вид МО ===
    if stack[-1] == "mm_species_mo":
        if text == "2024 год вид МО":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/WWIGB7MKm7gzsg")
        elif text == "2025 год вид МО":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/CaQLYpnF_0LNYg")
        mk = ReplyKeyboardMarkup(build_menu(MM_SPECIES_MO_MENU), resize_keyboard=True)
        await update.message.reply_text("Выберите год:", reply_markup=mk)
        return

    # === Профили резистентности ===
    if stack[-1] == "mm_resist":
        if text == "2024 пр резист":
            link = "https://disk.yandex.ru/i/0bnwY1XiGqPXcA"
            await update.message.reply_text(f"📄 Профили резистентности за 2024 г.:\n{link}")
        elif text == "2025 пр резист":
            link = "https://disk.yandex.ru/i/2Da4WudpqRGhZQ"
            await update.message.reply_text(f"📄 Профили резистентности за 2025 г.:\n{link}")
        return

    # === ESCAPE-патогены ===
    if stack[-1] == "mm_escape":
        if text == "2024 Escape":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/urX5sUQJ_qAIag")
        elif text == "2025 Escape":
            await update.message.reply_text("Планируемая дата реализации 16.01.2026")
        mk = ReplyKeyboardMarkup(build_menu(MM_ESCAPE_MENU), resize_keyboard=True)
        await update.message.reply_text("Выберите год:", reply_markup=mk)
        return
    # === Аварийные ситуации ===
    if stack[-1] == "as_menu":
        if text == "Алгоритм мер личной профилактики":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/rbVJMXp8NoAtdA")
        elif text == "Алгоритм учета и регистрации аварийных ситуаций":
            link = "https://disk.yandex.ru/i/_aruvxBoI8v2Vg"
            await update.message.reply_text(f"📄 Алгоритм учета и регистрации аварийных ситуаций:\n{link}")
        elif text == "Состав укладки экстренной профилактики парентеральных инфекций":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/LMnm078f1DxFGA")
        elif text == "Дотестовое и послетестовое консультирование при экспресс тестировании на ВИЧ-инфекцию":
            stack.append("as_consult")
            mk = ReplyKeyboardMarkup(build_menu(AS_CONSULT_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип консультирования:", reply_markup=mk)
            return
        elif text == "Информирование о проведении тестирования на ВИЧ-инфекцию":
            link = "https://disk.yandex.ru/i/rQc09SM2SO7Ydg"
            await update.message.reply_text(f"📄 Информирование о проведении тестирования на ВИЧ:\n{link}")
        elif text == "Журнал учета аварийных ситуаций":
            link = "https://disk.yandex.ru/i/0k83VIDOZ52VQw"
            await update.message.reply_text(f"📄 Журнал учета аварийных ситуаций:\n{link}")
        elif text == "Мероприятия по снижению числа АС":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/mh_Z6sMi4yVGEA")
        else:
            mk = ReplyKeyboardMarkup(build_menu(AS_MENU), resize_keyboard=True)
            await update.message.reply_text("Пункт не найден.", reply_markup=mk)
            return

        # После отправки — возврат в меню аварийных ситуаций
        mk = ReplyKeyboardMarkup(build_menu(AS_MENU), resize_keyboard=True)
        await update.message.reply_text("Аварийные ситуации:", reply_markup=mk)
        return

    # === Консультирование по ВИЧ ===
    if stack[-1] == "as_consult":
        consult_map = {
            "Дотестовое консультирование": "https://disk.yandex.ru/i/SIeQBF9hGum-xw",
            "Послетестовое консультирование": "https://disk.yandex.ru/i/X2RxxiNSrWlmbw",
            "Схема послетестового консультирования при отрицательном результате": "https://disk.yandex.ru/i/VnxGAc1PIURY2w",
            "Схема послетестового консультирования при положительном результате": "https://disk.yandex.ru/i/a_4hZGf0BheXHg"
        }
        if text in consult_map:
            await update.message.reply_photo(photo=consult_map[text])
        else:
            mk = ReplyKeyboardMarkup(build_menu(AS_CONSULT_MENU), resize_keyboard=True)
            await update.message.reply_text("Выберите тип консультирования:", reply_markup=mk)
            return

        mk = ReplyKeyboardMarkup(build_menu(AS_CONSULT_MENU), resize_keyboard=True)
        await update.message.reply_text("Выберите тип консультирования:", reply_markup=mk)
        return

    # === ООИ: Главное меню ===
    if stack[-1] == "oOI_menu":
        if text == "Противоэпидемические мероприятия":
            stack.append("oOI_measures")
            mk = ReplyKeyboardMarkup(build_menu(OOI_MEASURES_MENU), resize_keyboard=True)
            await update.message.reply_text("Противоэпидемические мероприятия:", reply_markup=mk)
        elif text == "СИЗ для ООИ":
            stack.append("oOI_ppe")
            mk = ReplyKeyboardMarkup(build_menu(OOI_PPE_MENU), resize_keyboard=True)
            await update.message.reply_text("СИЗ для ООИ:", reply_markup=mk)
        elif text == "Лечение и экстренная профилактика при ООИ":
            stack.append("oOI_treatment")
            mk = ReplyKeyboardMarkup(build_menu(OOI_TREATMENT_MENU), resize_keyboard=True)
            await update.message.reply_text("Лечение и профилактика:", reply_markup=mk)
        elif text == "Меры личной профилактики медицинских работников":
            stack.append("oOI_personal")
            mk = ReplyKeyboardMarkup(build_menu(OOI_PERSONAL_MENU), resize_keyboard=True)
            await update.message.reply_text("Меры личной профилактики:", reply_markup=mk)
        elif text == "Забор биоматериала при подозрении на ООИ":
            link = "https://disk.yandex.ru/d/CndmqGbpp21uaA"
            await update.message.reply_text(f"📄 Забор биоматериала при подозрении на ООИ:\n{link}")
            mk = ReplyKeyboardMarkup(build_menu(OOI_MENU), resize_keyboard=True)
            await update.message.reply_text("Особо опасные инфекции:", reply_markup=mk)
        elif text == "Режимы обеззараживания поверхностей при ООИ":
            stack.append("oOI_decont")
            mk = ReplyKeyboardMarkup(build_menu(OOI_DECONT_MENU), resize_keyboard=True)
            await update.message.reply_text("Режимы обеззараживания:", reply_markup=mk)
        else:
            mk = ReplyKeyboardMarkup(build_menu(OOI_MENU), resize_keyboard=True)
            await update.message.reply_text("Пункт не найден.", reply_markup=mk)
        return

    # === ООИ: Противоэпидемические мероприятия ===
    if stack[-1] == "oOI_measures":
        if text == "Общий план мероприятий":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/rrQERzTebQHwvQ")
        elif text == "Первичные мероприятия":
            link = "https://disk.yandex.ru/i/ObKRV28B61xbxA"
            await update.message.reply_text(f"📄 Первичные мероприятия:\n{link}")
        return

    # === ООИ: СИЗ для ООИ ===
    if stack[-1] == "oOI_ppe":
        if text == "Виды костюмов":
            stack.append("oOI_ppe_types")
            mk = ReplyKeyboardMarkup(build_menu(OOI_PPE_TYPES_MENU), resize_keyboard=True)
            await update.message.reply_text("Виды костюмов:", reply_markup=mk)
            return
        elif text == "Случаи применения костюмов":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/rSl9Io5s5wvjXw")
        elif text == "Порядок надевания и снятия костюма «1 типа»":
            link = "https://disk.yandex.ru/i/oXbLPciTOuxXbA"
            await update.message.reply_text(f"📹 Порядок надевания и снятия костюма «1 типа»:\n{link}")
        elif text == "Порядок надевания и снятия «Кварц-М1»":
            link = "https://disk.yandex.ru/i/ZVOyGYdfU9R_pw"
            await update.message.reply_text(f"📹 Порядок надевания и снятия «Кварц-М1»:\n{link}")
        mk = ReplyKeyboardMarkup(build_menu(OOI_PPE_MENU), resize_keyboard=True)
        await update.message.reply_text("СИЗ для ООИ:", reply_markup=mk)
        return

    # === ООИ: Виды костюмов ===
    if stack[-1] == "oOI_ppe_types":
        costume_images = {
            "Костюм 1 типа": "https://disk.yandex.ru/i/8xxF32G8oFKTwQ",
            "Костюм 2 типа": "https://disk.yandex.ru/i/zxVzw1sN2SrxkA",
            "Костюм 3 типа": "https://disk.yandex.ru/i/XENHeLAwPFpN_w",
            "Костюм 4 типа": "https://disk.yandex.ru/i/04kYBend1FNo2Q",
            "Костюм Кварц-М1": "https://disk.yandex.ru/i/uwgHGfIPR14YoA",
        }
        if text in costume_images:
            await update.message.reply_photo(photo=costume_images[text])
        mk = ReplyKeyboardMarkup(build_menu(OOI_PPE_TYPES_MENU), resize_keyboard=True)
        await update.message.reply_text("Виды костюмов:", reply_markup=mk)
        return

    # === ООИ: Лечение и профилактика ===
    if stack[-1] == "oOI_treatment":
        treatment_images = {
            "Схема терапии больных в критическом состоянии": "https://disk.yandex.ru/i/c5BJ1RzfzTm7Xw",
            "Схема профилактики при неизвестном возбудителе": "https://disk.yandex.ru/i/brdDSQlJrGdHQw",
            "Чума": "https://disk.yandex.ru/i/nIAaqMwANXenRg",
            "Холера": "https://disk.yandex.ru/i/AZeP_WsXb1cWyA",
            "Патогенный грипп": "https://disk.yandex.ru/i/5AuOnhIS6p4NTA",
            "Тропическая малярия": "https://disk.yandex.ru/i/vDaIXr7VGjG2Lw",
            "Лихорадка Рифт-Валли": "https://disk.yandex.ru/i/5vJ6__AZludE6g",
            "Менингококковая болезнь": "https://disk.yandex.ru/i/NRcF0SrH74EKYQ",
        }
        if text in treatment_images:
            await update.message.reply_photo(photo=treatment_images[text])
        mk = ReplyKeyboardMarkup(build_menu(OOI_TREATMENT_MENU), resize_keyboard=True)
        await update.message.reply_text("Лечение и профилактика:", reply_markup=mk)
        return

    # === ООИ: Меры личной профилактики ===
    if stack[-1] == "oOI_personal":
        if text == "Меры":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/eL6x615eHTLGQw")
        elif text == "Укладка экстренной профилактики ООИ":
            await update.message.reply_photo(photo="https://disk.yandex.ru/i/3kEDdz-r3fDcBw")
        mk = ReplyKeyboardMarkup(build_menu(OOI_PERSONAL_MENU), resize_keyboard=True)
        await update.message.reply_text("Меры личной профилактики:", reply_markup=mk)
        return

    # === ООИ: Режимы обеззараживания ===
    if stack[-1] == "oOI_decont":
        decont_images = {
            "Холера": "https://disk.yandex.ru/i/peO08GEA0LRpiA",
            "Чума": "https://disk.yandex.ru/i/djaYsTSyuj7MzA",
            "Сибирская язва": "https://disk.yandex.ru/i/r14qWfOdwiOpSg",
            "Фекалии, мокрота, сыворотка крови": "https://disk.yandex.ru/i/jurg6nsQibLtGA",
            "Вирусные инфекции": "https://disk.yandex.ru/i/lmVpT4cZVXPuRA",
            "Приготовление раствора «Хлорапин»": "https://disk.yandex.ru/i/jIIPqqzj2cE9Rg",
        }
        if text in decont_images:
            await update.message.reply_photo(photo=decont_images[text])
        mk = ReplyKeyboardMarkup(build_menu(OOI_DECONT_MENU), resize_keyboard=True)
        await update.message.reply_text("Режимы обеззараживания:", reply_markup=mk)
        return

# === ЗАПУСК ===
def main():
    TOKEN = "8375816364:AAEJayebQrIrE1FxITjVQqaE-O3GfHWnjM0"
    app = Application.builder().token(TOKEN).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    print("✅ Бот запущен. Напишите /start")
    app.run_polling()


if __name__ == '__main__':
    main()
